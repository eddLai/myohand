...
- [ ] **`deg` 要逐幀記，重放照著走**。Deadband 滑桿是邊跑邊調的（上面那張表
      就這樣寫），但 `meta` 只記開場那一個值，滑桿一動重放就對不上——實測
      900 幀動一次，470/900 不符。**這條最該先做**，不然下一趟 run 又是一份
      `DOES NOT MATCH`
- [ ] **replay check 不要跳過 `mode=off` 的幀**。`sent_*` 在濾波器關著時照樣
      記（`filt.update` 是無條件呼叫的），所以整段關著飛的 run 現在一次檢查
      都沒做，而拿掉那個 gate 成本是零
- [ ] **換相機的分支補 `last_sent = None`**，跟 `run_calibration` 對齊
