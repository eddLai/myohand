...
int main(void)
{
   ...
   for (i = 7; i <= 12; i++) out[i] = 500;
   for (i = 13; i <= 18; i++) out[i] = 500;
   show("before-open");
   cyc(3000);
   show("after-open ");

   out[1] = 1000; out[2] = 1000; out[3] = 0;    /* pinky+ring flex, middle straight */
   out[4] = 1000; out[5] = 1000; out[6] = -1;   /* index+thumb flex */
   cyc(3000);
   show("MIDDLE-UP  ");
   ecx_close(&ctx);
   return 0;
