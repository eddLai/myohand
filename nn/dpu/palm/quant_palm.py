...
eval_path = sys.argv[5] if len(sys.argv) > 5 else calib_path
raw = np.load(calib_path)
x = raw.astype(np.float32) / 255.0        # the graph is NHWC; its own transpose follows
# calibration may be augmented while the comparison stays on recorded frames,
# so the numbers remain answerable against every earlier run
ev = np.load(eval_path).astype(np.float32) / 255.0
print("calib %s  eval %s  range %.3f-%.3f" % (str(x.shape), str(ev.shape), x.min(), x.max()))
...
with torch.no_grad():
    ref = [model(torch.from_numpy(ev[i:i + 1])) for i in range(len(ev))]
...
with torch.no_grad():
    got = [q.quant_model(torch.from_numpy(ev[i:i + 1])) for i in range(len(ev))]
